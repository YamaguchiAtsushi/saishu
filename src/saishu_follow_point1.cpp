...
void scan_callback(const geometry_msgs::Point::ConstPtr &msg) {
    poll_x = msg->x;
	poll_y = msg->y;
...
{
	// 初期化関連
	ros::init(argc, argv, "saishu_follow_point1");
	ros::NodeHandle nh;
	ros::NodeHandle pnh("~");
	...
	ros::Subscriber poll_sub = nh.subscribe("/obstacle_robot", 1000, scan_callback);
